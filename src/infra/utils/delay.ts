...
export async function delay (
  timeInMs: number = DEFAULT_TEN_SECONDS,
  showLogs = true
) {
  if (showLogs) {
    Logger.info(
    ...
    Logger.info(
      `[PRE START]: Set a higher number on 'delay' function call on 'server.ts' if the server did not start correctly due container initialization delay, causing database connection fail.`
    );
  }
